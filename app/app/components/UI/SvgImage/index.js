			...
			if (uri.match(/^data:image\/svg/)) {
				...
				try {
					...
					if (text.toLowerCase().indexOf('<svg') !== -1) {
						...
						console.log('invalid svg', { text, uri });
						this.mounted && props.onError && props.onError('invalid svg');
					}
				} catch (err) {
					console.log('error loading remote svg image', err);
	...
	render() {
		...
		if (svgContent) {
			...
			if (svgContent.includes('viewBox')) {
				...
				const patchedSvgContent = `${svgContent.substr(0, 5) +
			...
			return (
				<WebView
					...
					style={[
						...
						{ display: this.state.loaded || Device.isAndroid() ? 'flex' : 'none' }
					]}
				/>
			);
		}
		return <View pointerEvents="none" style={[props.containerStyle, props.style]} />;
	}
}
