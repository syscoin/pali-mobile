...
import AvaxConfig from '../AvaxConfig.json';
import { logWarn, queryEIP1559Compatibility } from '../util';
import { NetworkConfig, NetworkState } from './NetworkController';

const mainChainId = '43114';
export class AvaxNetworkController extends BaseController<NetworkConfig, NetworkState> {
  ...
  private refreshNetwork() {
    ...
    this.initEIP1559Compatibility().catch((e) => {
      logWarn('PPYang, AvaxNetworkController getEIP1559Compatibility e', e);
    });
  }
  ...
  name = 'AvaxNetworkController';

  provider: any;

  constructor(config?: Partial<NetworkConfig>, state?: Partial<NetworkState>) {
    super(config, state);
    const { provider } = this.avaxNetworkConfig(mainChainId);
  ...
  setProviderType(type: string) {
    const { provider } = this.avaxNetworkConfigByType(type);
  ...
  avaxNetworkConfig(chainId: string) {
    const avaxConfig: { [index: string]: any} = AvaxConfig;
    for (const i in avaxConfig) {
      if (avaxConfig[i].provider.chainId === chainId) {
        return avaxConfig[i];
      }
    }
    return {};
  ...
  avaxNetworkConfigByType(type: string) {
    const avaxConfig: { [index: string]: any} = AvaxConfig;
    return avaxConfig[type];
  }
...
export default AvaxNetworkController;
